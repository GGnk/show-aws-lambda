...
const basicAuthorizer = async (event, _context, callback: APIGatewayAuthorizerCallback) => {
  ...
  try {
    ...
    const effect =
      !process.env[username] || process.env[username] !== password ? Effect.Deny : Effect.Allow;
